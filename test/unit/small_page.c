...
static const char*
test_small_page_list_(t_small_page** pages) {
    (void)pages;
    t_small_page page1 = {
        .block_list = NULL,
        .next = NULL,
    ...
    t_small_page* list = NULL;

    // test adding elements
    add_to_small_page_list(&list, &page1);
    if (list != &page1) {
        return "Adding to empty list failed";
    }
    add_to_small_page_list(&list, &page2);
    ...
    t_small_page* page_ptr = delete_from_small_page_list(&list, &page1);
    if (page_ptr != &page1 || list != &page2 ||
        list->next != NULL || list->prev != NULL
    ) {
        return "Removing from not empty list failed";
    }
    page_ptr = delete_from_small_page_list(&list, &page2);
...
static const char*
test_create_small_page_(t_small_page** pages) {
...
static const char*
test_find_in_small_page_list_(t_small_page** pages) {
    t_small_page* page_list = *pages;
    t_small_block* block = page_list->block_list;
    t_small_page* page = page_list;

    if (find_in_small_page_list((void*)block, page_list) != page) {
    ...
    if (find_in_small_page_list(invalid_ptr, page_list) != NULL) {
        return "invalid pointer found in page list";
    } else if (find_in_small_page_list(NULL, page_list) != NULL) {
...
static const char*
test_destroy_small_page_(t_small_page** pages) {
    t_small_page* page = (*pages)->next;
    for (size_t i = 3; i < NUM_PAGES; ++i) {
        t_small_page* next = page->next;
        t_small_page* prev = page->prev;
        int save_errno = errno;
        if (destroy_small_page(page, pages) == false) {
...
void test_small_page(struct s_heap* heap) {
    ...
    execute_test_(test_small_page_list_, "small page list add / remove: ", NULL);

    execute_test_(test_create_small_page_, "create small pages: ", pages);

    execute_test_(test_find_in_small_page_list_, "find in small page list: ", pages);

    execute_test_(test_destroy_small_page_, "destroy small page: ", pages);
}
